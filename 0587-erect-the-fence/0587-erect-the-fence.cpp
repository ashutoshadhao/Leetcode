class Solution
{
    public:
        struct point
        {
            ...
            bool operator==(point & O)
            {
            	// operator overloading for unique() func
                return (x == O.x && y == O.y);
            }
        };
    ...
    void convec_hull(vector<point> &p)
    {
        ...
        for (i = 1; i < n; i++)
        {
            ...
            {
            	// check if not counter clock wise (it consider both clock wise and collinear pts)
                while (up.size() >= 2 && ccw(up[up.size() - 2], up[up.size() - 1], p[i]))
                {
                	// check angle b/w (2nd last pt, last pt, current pt) from up vector
                    up.pop_back();
                }
                up.push_back(p[i]);
            }
            ...
            {
            	// check if not clock wise  (it consider both counter clock wise and collinear pts)
                while (down.size() >= 2 && cw(down[down.size() - 2], down[down.size() - 1], p[i]))
                {
                	// check angle b/w (2nd last pt, last pt, current pt) from down vector
                    down.pop_back();
                }
                down.push_back(p[i]);
            }
